...
def build_sql_prompt(
    ...
    return "\n".join(
        [
            ...
            "13. 「一级」「二级」「三级」「一级能效」等描述等级的词语，对应「能效等级」列（INTEGER，值1/2/3），不要与「能效比_制冷」「能效比_制热」（REAL，值如3.6/4.0）混淆。",
            "14. 列名中「_W」「_kg」「_mm」「_kWh」「_km_h」「_mAh」等后缀是列名的组成部分，不可省略。例如「制冷量_W」不可简化为「制冷量」。",
            "15. 当 WHERE 条件有多个值时，优先使用 IN (...) 而不是多个 OR。",
            "",
            "用户问题：",
            question.strip(),
            "",
